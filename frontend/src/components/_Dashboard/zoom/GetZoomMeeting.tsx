...
const MeetingTable: React.FC<MeetingTableProps> = ({ meetings }) => {
  ...
  const rows = meetings?.map(
    ({ title, start_time, start_date, meeting_invitees, duration, id }) => {
  ...
  const month = new Date(meeting?.start_date!).getMonth() + 1; // Adding 1 because getMonth() returns 0-11
      ...
      >
        <div>
          ...
          <p className="text-sm font-medium">
            Date de début :<span>{`${day} ${frenchMonth[month]} ${year}`}</span>
          </p>
          <p className="text-sm font-medium">
        ...
        <div className="flex flex-col justify-start">
          ...
          <ul className="flex flex-col justify-start">
            {meeting?.meeting_invitees.map(({ directus_users_id: user }) => (
              <li className="py-1">
...
const GetZoomMeeting = () => {
  ...
  async function getMeetingFromDirectus(token: string) {
    const meetings = await getDirectusMeeting(token);
    meetings && setMeetings(meetings);
    return meetings;
  }

  useEffect(() => {
    token?.acess_token && getMeetingFromDirectus(token.acess_token);
  }, []);
