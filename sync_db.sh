...
docker exec -t $LOCAL_DB_CONTAINER pg_dump -U $LOCAL_DB_USER -F p $LOCAL_DB_NAME > $DUMP_FILE
...
ssh $REMOTE_USER@$REMOTE_HOST << EOF
    ...
    export PGPASSWORD=directus; psql -h localhost -U $REMOTE_DB_USER -d $REMOTE_DB_NAME -f /tmp/$DUMP_FILE
    ...
    docker exec -t $REMOTE_DB_CONTAINER psql -h localhost -U $REMOTE_DB_USER -d $REMOTE_DB_NAME -f /tmp/$DUMP_FILE
